...
def draw_board(surface, font):
    # Külső fém/drót gyűrű imitációja
    pygame.draw.circle(surface, SILVER, BOARD_CENTER, R_BOARD + 5)
...
def get_score(x, y):
    ...
    if R_TRIPLE_INNER < dist <= R_TRIPLE_OUTER:
        return base_score * 3
    if R_DOUBLE_INNER < dist <= R_DOUBLE_OUTER:
        return base_score * 2
    return base_score